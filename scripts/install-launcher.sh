...
REPOSITORY="${CODEX_WEB_GPT_REPOSITORY:-m4j2rpf766-crypto/gpt-web-codex}"
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest GPT Web Codex release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="gpt-web-codex-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/gpt-web-codex-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/GPT Web Codex.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/GPT Web Codex" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/GPT Web Codex.app"
  if pgrep -x "GPT Web Codex" >/dev/null 2>&1; then
    echo "Quit GPT Web Codex before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/GPT Web Codex.previous.app"
...
LIB_DIR="${CODEX_WEB_GPT_LIB_DIR:-$HOME/.local/lib/gpt-web-codex}"
BIN_DIR="${CODEX_WEB_GPT_BIN_DIR:-$HOME/.local/bin}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/GPT Web Codex.AppImage"
WRAPPER="$BIN_DIR/gpt-web-codex"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "GPT Web Codex\\.AppImage" >/dev/null 2>&1; then
  echo "Quit GPT Web Codex before updating it" >&2
  exit 1
fi
...
install -m 0644 "$ICON_SOURCE" "$ICON_DIR/gpt-web-codex.png"
DESKTOP_WRAPPER="$(printf '%s' "$WRAPPER" | sed \
  ...
  -e 's/\$/\\$/g' \
  -e 's/%/%%/g')"
cat > "$APPLICATIONS_DIR/gpt-web-codex.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=GPT Web Codex
Comment=ChatGPT Web planning with standalone Luna execution
Exec="$DESKTOP_WRAPPER"
Icon=gpt-web-codex
Terminal=false
Categories=Development;
StartupWMClass=gpt-web-codex
EOF
chmod 0644 "$APPLICATIONS_DIR/gpt-web-codex.desktop"
